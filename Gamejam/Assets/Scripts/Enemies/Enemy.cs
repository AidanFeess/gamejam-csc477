...
public class Enemy : MonoBehaviour
{
    ...
    void Start()
    {
        maxHP = maxHP * 1 + (math.clamp(GameController.Instance.GetCurrentWave() - 10, 0, 1000) / 1);
        currentHP = maxHP;
        speed = speed * 1 + (math.clamp(GameController.Instance.GetCurrentWave() - 10, 0, 1000) / 1);
        UpdateHPBar();
